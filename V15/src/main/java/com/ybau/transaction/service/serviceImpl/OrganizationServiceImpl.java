...
public class OrganizationServiceImpl implements OrganizationService {
    ...
    public ResponseData updateUsable(Map<String, Object> map, HttpServletRequest request) throws ParseException {
        ...
        if (add < add1) {
            return new ResponseData(400, "结算金额不可大于订单总金额", null);
        }
        ...
        if (BigDecimalUtil.sub(add, add1) == 0) {
            //如果此次结算已全部结算金额
    ...
    public ResponseData verifyLimit(int organizationId) {
        ...
        if (monies != null && monies.size() > 0) {
            for (Money money : monies) {
                //实际金额+运费等于订单实收金额
                //实收金额-已结算金额 等于未结算金额
                double add = BigDecimalUtil.sub(BigDecimalUtil.add(money.getActualMoney(), money.getFreight()), money.getSettlementAmount());
    ...
    public ResponseData paymentStatus(Map<String, Object> map, HttpServletRequest request) throws ParseException, IllegalAccessException {
        ...
        double add1 = BigDecimalUtil.add(div, order.getSettlementAmount());//结算金额不可大于总金额
        double sub = BigDecimalUtil.sub(add, add1);
        if (sub < 0) {
            //如果订单总金额小于结算金额则返回错误
            return new ResponseData(400, "结算金额不可大于订单总金额", null);
        }
