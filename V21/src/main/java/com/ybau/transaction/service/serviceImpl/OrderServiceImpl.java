...
//    }
     ...
     */
    @Override
    public ResponseData leadSales(List<Map<String, Object>> orders, String uId) throws ParseException {
        SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        for (Map<String, Object> order : orders) {
            order.put("logUser", uId);
            order.put("logTime", df.parse(df.format(new Date())));
            order.put("logSubject", order.get("orderId"));
            order.put("logClassify", 7);
            if ((int)order.get("paymentMethod")==2){
                //如果为借样订单
                order.put("logRemarks", "已归还（借样）");
                order.put("logName", "实际归还时间：" + df.format(order.get("returnTime")));
                orderMapper.updateRetreatCargo((String) order.get("orderId"), 2);
                logMapper.saveProductLog(order);//录入归还日志信息
                orderMapper.updatePaymentStatus((String) order.get("orderId"), 5, null);
                expressMapper.updateTime((Date) order.get("returnTime"),(String) order.get("orderId"));//存入实际归还时间
                goodsService.updateWarehouse((List<Goods>) order.get("goodsList"));//回滚库存
                expressMapper.updateRetreat(order);//录入退货信息
                List<Goods> goodsList = (List<Goods>) order.get("goodsList");
                for (Goods goods : goodsList) {
                    goodsMapper.updateReturnNumber(goods.getReturnNumber(), goods.getId());
                }

            }else {
                //如果不为借样订单
                order.put("logRemarks", "退货");
                orderMapper.updateRetreat(order);//存入退款金额
                goodsService.updateWarehouse((List<Goods>) order.get("goodsList"));//回滚库存
                expressMapper.updateRetreat(order);//录入退货信息
                logMapper.saveProductLog(order);//录入归还日志信息
                List<Goods> goodsList = (List<Goods>) order.get("goodsList");
