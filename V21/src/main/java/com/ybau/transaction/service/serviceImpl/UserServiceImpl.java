        ...
        if (params.containsKey("username") && !StringUtils.isEmpty(params.get("username"))
            ...
            if (loginUser != null) {
                if (loginUser.getFlag() == 0) {
                        ...
                        for (Order order : orders) {
                            ...
                            if (i < RemindTime) {
                                ...
                                for (Goods goods : goodsDetail) {
                                    //判断下单商品总数量减去归还数量+转销售数量数量是否大于0  大于0 还有商品未归还
                                    if (goods.getGoodsNumber() - (goods.getSellNumber() + goods.getReturnNumber()) > 0) {
                                        remind = "您公司有借样订单还未归还，请尽快归还";
                                        //查询到有订单后结束循环
                                        break;
                                    }
                                }
                    ...
                    return new ResponseData(200,"登录成功",data);
                }
                return new ResponseData(400,"账户已冻结，请联系管理员激活",null);
            } else {
                //lock 小于1时 账户不锁定
                if (lock < 1) {
                    return new ResponseData(400,"账号或密码输入错误",null);
                } else {
                        mapper.updateByUserCount(username);
                        user = mapper.findByUserCount(username);
                        //超过设置次数更改状态冻结账户
                        if (user == null) {
                            return new ResponseData(400,"此账户不存在",username);
                        }
                        if (user.getUserCount() > lock) {
                            mapper.updateFlag(2, username);
                            return new ResponseData(400,"账户已冻结，请联系管理员激活",username);
                        }
                        return new ResponseData(400,"您密码已连续输错" + user.getUserCount() + "次，超过" + lock + "次将会锁定",username);
                }
            }
        }
        return new ResponseData(400,"账号或者密码为空",null);
    }
